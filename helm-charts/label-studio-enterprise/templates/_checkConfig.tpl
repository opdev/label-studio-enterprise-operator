...
- `define` a new template, prefixed `lse.checkConfig.`
- Check for known problems in configuration, and directly output messages (see message format below)
- Add a line to `lse.checkConfig` to include the new template.
...
*/}}
{{- define "lse.checkConfig" -}}
{{- $messages := list -}}
{{/* add templates here */}}

{{/* _checkConfig_lse_shell.tpl*/}}
{{/*{{- $messages = append $messages (include "lse.checkConfig.lseShell.proxyPolicy" .) -}}*/}}

{{/* other checks */}}
{{- $messages = append $messages (include "lse.checkConfig.pgConfig" .) -}}
...
{{- $messages = append $messages (include "lse.checkConfig.labelStudioHostisSet" .) -}}
{{- $messages = append $messages (include "lse.checkConfig.labelStudioHostScheme" .) -}}
{{- $messages = append $messages (include "lse.checkConfig.persistenceEnabled" .) -}}
{{- $messages = append $messages (include "lse.checkConfig.persistenceMinioNoConflict" .) -}}
{{- $messages = append $messages (include "lse.checkConfig.azureConfig" .) -}}
{{- $messages = append $messages (include "lse.checkConfig.gcsConfig" .) -}}
{{- $messages = append $messages (include "lse.checkConfig.s3Config" .) -}}
{{- $messages = append $messages (include "lse.checkConfig.featureFlagsFFprefix" .) -}}
{{- $messages = append $messages (include "lse.checkConfig.pGandRedisCIonly" .) -}}
...
{{- define "lse.checkConfig.labelStudioHostisSet" -}}
...
{{- end -}}
{{- end -}}
{{/* END lse.checkConfig.labelStudioHost */}}

{{/* Ensure that "LABEL_STUDIO_HOST" has scheme */}}
{{- define "lse.checkConfig.labelStudioHostScheme" -}}
...
{{- end -}}
{{- end -}}
{{/* END lse.checkConfig.labelStudioHostScheme */}}
...
{{- end -}}
{{- end -}}
{{/* END lse.checkConfig.redisHost */}}

{{/* Ensure that postgresql host is set */}}
{{- define "lse.checkConfig.pgConfig" -}}
{{- if (not .Values.postgresql.enabled) -}}
{{- if (not .Values.global.pgConfig.host) -}}
lse-app:
  PostgreSQL: PostgreSQL is required for Label Studio Enterprise. Please set PostgreSQL host in `.Values.global.pgConfig.host`
{{- end -}}
...
{{- end -}}
{{- end -}}
{{/* END lse.checkConfig.redisSslscheme */}}
...
{{- end -}}
{{- end -}}
{{/* END lse.checkConfig.persistenceEnabled */}}

{{/* Ensure no conflicts in minio/persistence upgrade*/}}
{{- define "lse.checkConfig.persistenceMinioNoConflict" -}}
{{- if and (and .Values.global.persistence.enabled .Values.minio.enabled) (not (hasKey .Values.global.extraEnvironmentVars "MINIO_MIGRATION") ) -}}
lse-app:
  Persistence: Looks like you're in the middle of migration process from `minio` to a `persistent storage`.
  Please, check our documentation for further steps: https://labelstud.io/guide/decommission_minio.html
{{- end -}}
{{- end -}}
{{/* END lse.checkConfig.persistenceMinioNoConflict */}}

{{/* Ensure that s3Config is correctly set */}}
{{- define "lse.checkConfig.s3Config" -}}
...
{{- end -}}
{{- end -}}
{{/* END lse.checkConfig.s3Config */}}

{{/* Ensure that azureConfig is correctly set */}}
{{- define "lse.checkConfig.azureConfig" -}}
...
{{- end -}}
{{- end -}}
{{/* END lse.checkConfig.azureConfig */}}

{{/* Ensure that gcsConfig is correctly set */}}
{{- define "lse.checkConfig.gcsConfig" -}}
...
{{- end -}}
{{- end -}}
{{/* END lse.checkConfig.gcsConfig */}}

{{/* Ensure that feature flags has ff_ prefix */}}
{{- define "lse.checkConfig.featureFlagsFFprefix" -}}
{{- if .Values.global.featureFlags -}}
{{- range $key, $value := .Values.global.featureFlags -}}
{{- if not (hasPrefix "ff_" (printf "%s" $key)) }}
lse-app:
  Feature Flags: flags should starts from `ff_` in lower case. Please check spelling in `.Values.global.featureFlags`
{{- end -}}
{{- end -}}
{{- end -}}
{{- end -}}
{{/* END lse.checkConfig.featureFlagsFFprefix */}}

{{/* Ensure that PG and Redis are not used in Production */}}
{{- define "lse.checkConfig.pGandRedisCIonly" -}}
{{- if not .Values.ci -}}
...
{{- end -}}
{{- end -}}
{{/* END lse.checkConfig.pGandRedisCIonly */}}