...
if ! awk 'BEGIN{exit(!(ARGV[1] ~ /^[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}((\-hotfix\.[0-9]+)?)/))}' "$LSE_VERSION"; then
  exit 0
fi

LSE_VERSION_NO_SUFFIX=$(echo $LSE_VERSION | cut -d'-' -f1)
NEW_MAJOR_VERSION=$(echo $LSE_VERSION_NO_SUFFIX | awk -F "." '{print $1}')
NEW_MINOR_VERSION=$(echo $LSE_VERSION_NO_SUFFIX | awk -F "." '{print $1"."$2}')

NEW_CHART_MAJOR_VERSION=$(echo $CHART_VERSION | awk -F "." '{print $1}')
NEW_CHART_MINOR_VERSION=$(echo $CHART_VERSION | awk -F "." '{print $1"."$2}')

OLD_VERSION_STRING=$(cat /chart-info/lseVersion)
OLD_CHART_VERSION_STRING=$(cat /chart-info/lseChartVersion)
...
OLD_VERSION_NO_SUFFIX=$(echo $LSE_VERSION | cut -d'-' -f1)
...
if [ ${OLD_MAJOR_VERSION} -lt ${NEW_MAJOR_VERSION} ] || [ ${OLD_CHART_MAJOR_VERSION} -lt ${NEW_CHART_MAJOR_VERSION} ]; then
  if (! greater_version $OLD_MINOR_VERSION $MIN_VERSION) || (! greater_version $OLD_CHART_MINOR_VERSION $CHART_MIN_VERSION); then
    notify "It seems you are upgrading the Label Studio Enterprise Helm Chart from ${OLD_CHART_VERSION_STRING} (Label Studio Enterprise ${OLD_VERSION_STRING}) to ${CHART_VERSION} (Label Studio Enterprise ${LSE_VERSION})."
    ...
    notify "and upgrade to Label Studio Enterprise Helm Chart version ${CHART_MIN_VERSION} before upgrading to ${CHART_VERSION}."
    exit 1
  fi
fi
